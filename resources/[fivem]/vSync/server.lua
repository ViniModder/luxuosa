...
RegisterCommand('blackout', function(source)---deixa cidade sem lux 
	local source = source
	local user_id = vRP.getUserId(source)
    if source == 0 then
...
RegisterCommand('clima', function(source, args)
	...
	local user_id = vRP.getUserId(source)
    if vRP.hasPermission(user_id,'developer.permissao') then
        TriggerClientEvent('vSync:freezeClima', source, args)
    end
end, false)
...
RegisterCommand('tempo', function(source, args, rawCommand)
    ...
    if vRP.hasPermission(user_id,'admin.permissao') then
        TriggerClientEvent('vSync:freezeTempo', source, args)
    end
end)
